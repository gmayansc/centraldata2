...
public class PedidosEliminarControlador implements Initializable {
	...
	private void eliminarPedido(ActionEvent event) {
		...
		try {
	        ...
	        Pedido ped;
	        ped = PedidoControlador.getPedido(Integer.parseInt(this.txtPedidoID.getText()));
	        
	        if (ped == null) {
	        	Alert alert = new Alert(Alert.AlertType.ERROR);
				alert.setHeaderText(null);
				alert.setTitle("Error");
				alert.setContentText("El pedido no existe y no puedes borrarlo");
				alert.showAndWait();
	        } else {
